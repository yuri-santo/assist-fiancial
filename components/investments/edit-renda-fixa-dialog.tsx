  ...
  useEffect(() => {
    if (formData.valor_investido && formData.taxa && formData.data_vencimento && formData.data_aplicacao) {
      ...
      if (dias > 0 && valorInicial > 0) {
        let taxaAnual = 0
        if (formData.indexador === "cdi") taxaAnual = (CDI_ATUAL * taxa) / 100
        else if (formData.indexador === "ipca") taxaAnual = IPCA_ATUAL + taxa
        else if (formData.indexador === "prefixado") taxaAnual = taxa
        else if (formData.indexador === "selic") taxaAnual = (CDI_ATUAL * taxa) / 100
        else taxaAnual = taxa
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await supabase
      ...
      .update({
        ...
        valor_investido: Number.parseFloat(formData.valor_investido),
        valor_atual: Number.parseFloat(formData.valor_atual),
